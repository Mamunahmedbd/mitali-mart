				...
				<?php
                    if (!is_object($args['cat']) && $product->is_purchasable() && $product->is_in_stock()) {
                        echo apply_filters( 'woocommerce_loop_add_to_cart_link',
                            ...
                            . '">'
                            . ($hover == 'shop_buttons' ? esc_html__('Buy now', 'custom-made') : '')
                            . '</a>',
                            $product );
                    }
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link icon-link"><?php
					if ($hover == 'shop_buttons') esc_html_e('Details', 'custom-made');
					?></a>
            </div>
			<?php
