		...
		if ( $args['singular'] ) {
				...
				<div class="post_featured<?php if ($args['class']) echo ' '.esc_attr($args['class']); ?>"<?php if ($args['seo']) echo ' itemscope itemprop="image" itemtype="//schema.org/ImageObject"'; ?>>
					...
					if (!$args['thumb_bg']) {
						if ( has_post_thumbnail() ) {
							...
							?><img<?php if ($args['seo']) echo ' itemprop="url"'; ?> src="<?php echo esc_url(custom_made_get_no_image_placeholder()); ?>" alt="<?php the_title_attribute(); ?>"><?php
						}
					}
					?>
				</div><!-- .post_featured -->
			...
			if ($has_thumb || !empty($args['show_no_image']) || (!$args['thumb_only'] && in_array($post_format, array('gallery', 'image', 'audio', 'video')))) {
				...
				if ( $args['thumb_bg'] ) {
					...
					?>
					<img src="<?php echo esc_url(custom_made_get_no_image_placeholder()); ?>" alt="<?php the_title_attribute(); ?>">
					<div class="mask"></div>
					<?php 
					custom_made_hovers_add_icons($args['hover']);
				}
